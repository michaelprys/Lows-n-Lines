...
const { messageData, issues, sendMessage, pending } = useFormSubmission();
</script>
...
<template>
    <form class="mt-24 xl-max:mt-16" @submit.prevent="onSubmit" novalidate>
            ...
            <div :class="isVisible ? 'fade-in' : 'invisible'">
                ...
                <ul class="mt-7 grid grid-cols-2 gap-x-7 gap-y-5 *:flex *:flex-col">
                    ...
                    <li>
                        ...
                        <ItemInputMessage :fieldName="'phoneNumber'" :issues="issues" />
                    </li>
                    <li>
        ...
        <ItemObserver v-slot="{ isVisible }">
            <ButtonCommon :class="isVisible ? 'fade-in-up' : 'invisible'" @click="sendMessage()" :pending="pending"
                >Send message
            </ButtonCommon>
        </ItemObserver>
    </form>
